...
//!
//! - `UPPER_ALIASES` — a `phf::Map` of ftfy's ALL-CAPS HTML entity aliases
//!   (`&EACUTE;` → `É`), derived from `htmlize::ENTITIES`. Replaces a ~500 µs
//!   lazy_static FxHashMap init.
...
//! - `lookup_width` — a `const fn` `match` folding fullwidth/halfwidth
//!   characters to their NFKC standard form for `fix_character_width`. It's a
//!   match rather than a map because it's probed once per input char, and a
//!   `match` lowers to branch comparisons with no hashing or allocation.
//!
//! All are emitted into `chardata_generated.rs` and surfaced through the
//! `chardata_generated` module.
...
fn main() {
    ...
    write_generated_tables(&out_dir.join("chardata_generated.rs"));
}

fn write_html_entity_overlay(out: &mut impl Write) {
    ...
    writeln!(
        out,
        "/// ftfy ALL-CAPS HTML entity aliases (e.g. `&EACUTE;`) that aren't in the\n\
         /// WHATWG set, mapped to their decoded value. Generated by `build.rs`\n\
         /// from `htmlize::ENTITIES`."
    )
    .unwrap();
    ...
    writeln!(
        out,
        "pub static UPPER_ALIASES: phf::Map<&'static str, &'static str> = {};",
        builder.build()
    )
    .unwrap();
}
...
/// module: the `UPPER_ALIASES` HTML-entity map,
/// `UTF8_CONTINUATION_STRICT_SET`, and the `lookup_width` width-folding match.
/// All are pure functions of static data, so we build them here rather than on
/// first use.
fn write_generated_tables(out_path: &std::path::Path) {
    let mut out = BufWriter::new(File::create(out_path).unwrap());

    write_html_entity_overlay(&mut out);
    write_strict_continuation_set(&mut out);
    write_width_match(&mut out);
}

/// Emit a `const fn lookup_width(char) -> Option<char>` whose arms fold each
/// fullwidth/halfwidth character to its NFKC standard form — mirroring the
/// runtime construction ftfy does with `unicodedata.normalize`. A `match`
/// (rather than a `phf`/`FxHashMap`) keeps `fix_character_width`'s per-char
/// probe branch-only, with no hashing.
fn write_width_match(out: &mut impl Write) {
    // BTreeMap for stable, sorted codegen output (lets rustc build a tight
    // comparison tree over the arms).
    let mut width_map: BTreeMap<char, char> = BTreeMap::new();
    ...
    width_map.insert('\u{3000}', ' ');

    for i in 0xFF01..0xFFF0 {
        if let Some(ci) = char::from_u32(i) {
            if let Some(c) = ci.nfkc().next() {
                if c != ci {
                    width_map.insert(ci, c);
                }
            }
        }
    }
    ...
    writeln!(
        out,
        "/// Fold a fullwidth/halfwidth character to its NFKC standard form,\n\
         /// used by `fix_character_width`. Generated by `build.rs`."
    )
    .unwrap();
    writeln!(out, "pub const fn lookup_width(c: char) -> Option<char> {{").unwrap();
...
fn write_strict_continuation_set(out: &mut impl Write) {
